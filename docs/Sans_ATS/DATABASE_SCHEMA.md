...
- `db/schema_init.sql` : schéma canonique de bootstrap local.
- `db/migrations/001_initial_schema.sql` : migration initiale complète (fusion de l'ancienne 002).
...
Base : `db/recruitment_assistant.db`  
Nombre de tables : **9**

| Table | Rôle |
|-------|------|
| `offers_raw` | Offres brutes + sections extraites |
| `offer_keywords` | Keywords extraits par LLM |
| `my_experiences` | Expériences personnelles |
| `my_projects` | Projets personnels |
| `formations` | Formations académiques enrichies |
| `matching_scores` | Matching offre ↔ expériences/projets |
| `formation_matching_scores` | Matching offre ↔ formations |
| `generations` | Traces des artefacts générés |
| `archive_manifest` | Index d'archivage |
...
### Recuperer offre + keywords + matching
...
SELECT
    ...
    o.tier,
    k.keywords_json,
    GROUP_CONCAT(m.match_id || '|' || COALESCE(m.exp_id, m.project_id) || '|' || m.match_score) AS matches
FROM offers_raw o
...
### Top matching experiences pour une offre
...
### Top matching formations pour une offre
...
SELECT
    ...
    fm.match_score,
    fm.reasoning
FROM formation_matching_scores fm
JOIN formations f ON fm.formation_id = f.formation_id
WHERE fm.offer_id = 'offer-20260405-123456'
ORDER BY fm.match_score DESC
LIMIT 5;
```

### Monitoring volume DB

```sql
SELECT 'offers_raw' AS table_name, COUNT(*) AS row_count FROM offers_raw
UNION ALL
...
UNION ALL
SELECT 'formation_matching_scores', COUNT(*) FROM formation_matching_scores
UNION ALL
...
1. Toute evolution de schema passe d'abord par `db/schema_init.sql`.
2. Ensuite, creer/mettre a jour la migration SQL correspondante dans `db/migrations/`.
3. Mettre a jour ce document dans le meme commit.
